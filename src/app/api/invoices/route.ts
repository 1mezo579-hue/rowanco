...
export async function POST(req: Request) {
  try {
    ...
    // If connection was closed, reconnect and try once more
    if (e.message?.includes("Server has closed the connection") || e.message?.includes("connection")) {
      try {
        await reconnectPrisma();
        return NextResponse.json({ error: "انقطع الاتصال بقاعدة البيانات، أعد المحاولة مرة واحدة" }, { status: 503 });
      } catch {}
    }
